...
class TestLoadingData(TestCase):
    edges = get_edges_with_costs(f"{RESOURCES}/test_clustering_1_8.txt")
...
class TestClusters(TestCase):
    ...
    def test_graph_2(self):
        edges = get_edges_with_costs(f"{RESOURCES}/test_clustering_16_64.txt")
        nodes, _, distances = to_graph(edges)
        tree, actual = clusters.kruskal(nodes, distances, num_clusters=4)
        expected = 216
        self.assertEqual(actual, expected)
    ...
    def test_big_graph(self):
        edges = get_edges_with_costs(f"{RESOURCES}/clustering1.txt")
        nodes, _, distances = to_graph(edges)
